...
class WorldBuilder:
    ...
    def build_biome_world(self, tile_size=4, grid_range=20):
        ...
        # We use positional arguments here because uvScaling keyword failed.
        # Order: shapeType, radius, halfExtents, length, fileName, meshScale, 
        # pos, ori, rgba, specular, flags, uvScaling
        ...
        for x in range(-grid_range, grid_range):
            for y in range(-grid_range, grid_range):
                ...
                # Create Tile
                # Inside the loop, before createMultiBody:
                random_yaw = random.choice([0, 1.57, 3.14, 4.71]) # 0, 90, 180, 270 degrees
                rand_ori = p.getQuaternionFromEuler([0, 0, random_yaw])

                # Then use it here:
                tile_id = p.createMultiBody(0, collision_id, use_v_id, [wx, wy, -0.1], rand_ori)

                # Apply specific texture and biome color
